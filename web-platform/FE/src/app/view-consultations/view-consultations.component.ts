...
export class ViewConsultationsComponent implements OnInit {
  ...
  token='eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..pRp6n0QdaaUbWQHA16Ar5dN1jF6u_x0KuwaV-CssLIUZBk0Bmg16idkFe6PR165X894EyxSc3aAFyVcOwAnZ_T5sdMuszbNjvzWNlVtSfJ71APqN2iu_fialNz0LzZU07fe-7PBxIO9XzZzcqZszCNn8_9Y3VEK2pfNEtpTsX2UMGNBoyWJQgEnFbPV5Tu8fuc78Rg-NhiiNGPPIaur-5bDJJRoRy5_wv0ctJKksSJzmmWncJ61jCGUa44As5ooSeiPm2nYxC8UkVF3BARQKBLKern4GsvEAhBRTmJlh7oRFvpC4NqESJbcWVeaPe4Xe4PwX0VIU47q6RvXfQEIOXg'
  ...
  constructor(private route: ActivatedRoute,
              ...
              private diseaseService: PatientService) // seems reused for diseases
  {
  ...
  ngOnInit(): void {
    ...
    if (id !== null) {
      this.patientService.getPatientById(id).subscribe(patient => {
        ...
        this.alertService.error('Patient was not found.');
      });
    }
  ...
  submitReferral(): void {
    if (this.referralForm.valid && this.patient?.id) {
      ...
      this.referralService.createReferral(referral).subscribe({
        ...
        error: () => {
          this.alertService.error('Error submitting the referral.');
        }
      });
    } else {
  ...
  saveEdit(): void {
    if (this.medicationForm.invalid || !this.patient?.id) {
      this.alertService.error('The form is invalid or the patient is not loaded yet.');
      return;
    }
    ...
    this.medicationService.createMedication(medication).subscribe({
      next: () => {
        this.alertService.success('Medication was saved successfully.');
      ...
      error: () => {
        this.alertService.error('Error saving the medication.');
      }
    });
  }
