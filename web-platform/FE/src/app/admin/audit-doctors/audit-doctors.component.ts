...
@Component({
  ...
  providers: [DatePipe],
  templateUrl: './audit-doctors.component.html',
  styleUrl: './audit-doctors.component.css'
})
export class AuditDoctorsComponent  implements OnInit {
  auditLogs: any[] = [];
  isLoading = false;
  ...
  loadAuditLogs(entity: string): void {
    ...
    this.auditService.getEntityAuditLogs(entity).subscribe({
      next: (response: any) => {
        this.auditLogs = response[entity]
          .map((log: any) => {
            const parsedDiffs = this.parseDiffs(log.diffs);
            return {
              ...log,
              parsedDiffs,
          ...
          .filter((log: { parsedDiffs: {}; }) => {
            const keys = Object.keys(log.parsedDiffs);
            if (keys.length === 0) {
              return false;
            }
            return !keys.every(key => key === 'lastLogin' || key === 'lastLoginAt');
          });
